...
public class MailPool {
	...
	private void loadItem(ListIterator<Robot> i) throws ItemTooHeavyException {
		...
		if(pool.size()>0) {
		    if(robot instanceof FastRobot) {
		    ...
		    } else if(robot instanceof RegularRobot) {
			   ...
			   /*Assign next 5 incoming mails to this bulk robot*/
			   try {
				   while(robot.getTubeSize()<5 && pool.size()>0) {
					   robot.addToTube(j.next().mailItem);
					   j.remove();
				   }
	 			   robot.dispatch();
